...
let client: ReturnType<typeof transport.session | typeof transport.stateless>;

describe.each(['stateless', 'session'])('%s protocol', (kind) => {
	beforeEach(async () => {
		if (kind === 'session') {
			...
			(package_manager) => {
				...
				describe.each([
					...
					it("doesn't suggest anything for packages that don't have replacements", async () => {
						const tool = await client.callTool('npm-i-checker', {
	...
	describe('code-checker', () => {
		...
		it("doesn't suggest anything for packages that don't have replacements", async () => {
			const tool = await client.callTool('code-checker', {
	...
	describe('lookup-replacement', () => {
		...
		it("doesn't return anything for empty queries", async () => {
			const tool = await client.callTool('lookup-replacement', {
				query: '   ',
			});
